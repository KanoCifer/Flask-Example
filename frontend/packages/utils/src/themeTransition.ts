...
function createTransitionIcon(targetTheme: ThemeMode, inkColor?: string): HTMLElement {
  ...
  if (targetTheme === 'dark') {
    ...
    container.innerHTML = `<div><svg viewBox="0 0 48 48" width="120" height="120" fill="none" stroke="${inkColor ?? '#f5df17'}" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="44" height="28" rx="2" ry="2"/><line x1="16" y1="42" x2="32" y2="42"/><line x1="24" y1="35" x2="24" y2="42"/></svg></div>`;
  } else {
...
export function playThemeTransition(
  ...
  const targetInkColor = getCssVar(
    isDark,
    scheme,
    '--ink',
  ...
  const icon = createTransitionIcon(targetTheme, targetInkColor);
