...
# （出 MissionBundle 练习）两套规范，并补充工具使用 / 研究 / 收尾说明。
...
COURSE_AGENT_INSTRUCTIONS = (
    ...
    "（MissionBundle）。所有写盘均由工具完成，不要自己构造文件路径。\n\n"
    "## 工具使用说明\n"
    ...
    "## 收尾说明\n"
    "- 所有工具调用完成后，最终响应必须**严格返回 ``MissionBundle`` JSON**"
    "（``missions`` 是 Mission 对象列表），不要包含解释文字 / Markdown / 代码围栏，"
    "只输出 JSON。\n"
...
COURSE_AGENT_USER_PROMPT_TEMPLATE = (
    ...
    "全部完成后，最终响应必须严格返回 ``MissionBundle`` JSON"
    "（``missions`` 是本课的练习任务列表，不要包含解释文字 / Markdown / 代码围栏）。"
)

# 课程 agent 整 run 兜底重试的修正指令（task-3554）：:meth:`LearningService.
# _generate_lesson` 在「磁盘缺 body 文件」或「MissionBundle 解析失败」时，把
# 本提示追加到用户消息末尾重跑一次，指示 agent 修正上次失败原因。
COURSE_AGENT_RETRY_HINT = (
    "## 上次生成失败，请修正后重新完成本课\n"
    "上一次 run 的结果未通过 service 校验（可能原因：漏调 ``save_lesson`` 导致 "
    "lesson body 未落盘，或最终响应无法解析为 ``MissionBundle``）。请严格做到：\n"
    "1. 必须先调用 ``save_lesson`` 把本课正文写盘，再输出练习；\n"
    "2. 最终响应**只**输出合法 ``MissionBundle`` JSON，不要包裹 Markdown 代码围栏，"
    "不要包含任何解释文字；\n"
    "3. 每个 mission 的 ``answer`` 类型必须与 ``type`` 严格一致（single_choice→str、"
    "multi_choice→list[str]、true_false→bool）；\n"
    "4. 每个 mission 的 ``explanation`` 必填。"
)
